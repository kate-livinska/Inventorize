...
class OrderItem(db.Model):
    ...
    order_id = db.Column(db.String(50), db.ForeignKey('order.id'), nullable=False)  # Added foreign key relationship
...
def get_orders():
    ...
    return jsonify({"results": [{"id": order_id} for order_id in order_ids]})
...
def get_order_details(order_id):
    ...
    items = order.items
    result = [{"id": item.id, "sku": item.sku, "quantity": item.quantity, "box": item.box} for item in items]
...
def handle_order_item(order_id, item_id):
    ...
    # Perform actual token, order_id, and item_id validation (for simplicity, assuming valid token, order_id, and item_id)
    # In a real-world application, you would use a proper authentication and authorization mechanism
    ...
    new_item = OrderItem(id=item_id, sku="example_sku", quantity=data["quantity"], box="example_box", order_id=order_id)
    db.session.add(new_item)
    db.session.commit()
    return jsonify({"id": new_item.id, "sku": new_item.sku, "quantity": new_item.quantity, "box": new_item.box})
...
def get_order_item(order_id, sku):
    ...
    result = {"id": item.id, "sku": item.sku, "quantity": item.quantity, "box": item.box}
...
def create_order():
    ...
    # Create a new order
    new_order = Order(id=generate_random_string())
