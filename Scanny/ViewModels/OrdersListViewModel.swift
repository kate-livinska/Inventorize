...
class OrdersListViewModel: ObservableObject {
    ...
    init() {
        isLoading = true
        guard let fetchedOrders = OrdersListViewModel.fetchOrdersData() else {
            ordersList = OrdersListViewModel.createOrdersList([
                Order(id: "error"),
                Order(id: "error2"),
                Order(id: "error3")
            ])
            print("Error: could not create orders cards.")
            return
        }
